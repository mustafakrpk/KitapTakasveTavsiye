...
<body>
        ...
        <div class="container">
            <nav class="main-nav">
                ...
                <ul class="main-nav__items">
                    ...
                    <li><a href="gelen_mesaj.php" class="active"> Gelen Mesajlar</a></li>';
                    }
                    ?>
    ...
    <div class="container mesajlasmaalani">
        ...
        $gelen_mesajlar = $db->query('SELECT * FROM mesajlar where alici_id=?', $_SESSION["id"])->fetchAll();
        ?>
        <table class='table'>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                foreach ($gelen_mesajlar as $gelen_mesaj) {
                    $db->query('SELECT * FROM user where id=?', $gelen_mesaj["gonderen_id"])->fetchAll(function ($account) {
                        echo $account['name'];
                    });
                    echo " <tr>
                    <th scope='row'>$gelen_mesaj[id]</th>";
                    $db->query('SELECT * FROM user where id=?', $gelen_mesaj["gonderen_id"])->fetchAll(function ($account) {
                        echo "<td>$account[name]</td>";
                    });
                    echo " <td>$gelen_mesaj[mesaj]</td>
                    <td><a href='mesajlas.php?alici_id=$gelen_mesaj[gonderen_id]&kitap_id=$gelen_mesaj[kitap_id]'>Cevapla</a></td>
                  </tr>";
                }
                ?>
